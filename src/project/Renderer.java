...
/**
 * třída pro vykreslení celé scény
 *
...
public class Renderer extends AbstractRenderer implements Disposable {
    ...
    private float cameraSpeed = 0.002f;
    ...
    public void init() {
        ...
        GluUtils.gluPerspective(60, width / (float) height, 0.001f, 100.0f); //transformace zobrazovacího objemu - perspektiva
    ...
    public void display() {
        ...
        if (wireframe) { //zapnutí/vypnutí drátového modelu
        ...
        glMatrixMode(GL_MODELVIEW); //modelovací matice
        glLoadIdentity();
        ...
        renderAxes(); //vytvoření os

        scene.update(deltaTime); //update scény

        scene.draw(); //vykreslení scény
