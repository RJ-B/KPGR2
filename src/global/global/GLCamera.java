...
public class GLCamera {
	...
	private boolean first_person; // true -> 1st person, false -> 3rd person
	private boolean valid; // true -> nemusí být prováděna metoda compute matrix, false -> před metodami get a set je zavolána metoda compute matrix
	...
	public void setMatrix() {
		if (!valid)
			compute_matrix();
		Vec3D e = new Vec3D(eye.getX(), eye.getY(), eye.getZ());
		Vec3D v = new Vec3D(centre.getX(), centre.getY(), centre.getZ());
		Vec3D u = new Vec3D(up.getX(), up.getY(), up.getZ());
		Mat4ViewRH.view(e, v, u);
